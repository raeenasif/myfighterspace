@forelse($roles->where('name', 'Trainers') as $role)
<div class="mb-2">
    <div class="form-check">
        <input name="roles[]" id="role_{{ $role->id }}" value="{{ $role->id }}" class="form-check-input" type="checkbox" {{ (old('rules') && in_array($role->id, old('rules'), true)) || (isset($user) && in_array($role->id, $user->roles->modelKeys(), true)) ? 'checked' : '' }} />

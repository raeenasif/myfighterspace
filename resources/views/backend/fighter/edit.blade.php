...
@section('content')
<form action="{{ route('admin.fighter.update', $user) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('patch')
    <x-backend.card>
        <x-slot name="header">
        ...
        <x-slot name="headerActions">
            <x-utils.link class="card-header-action" :href="route('admin.fighter.index')" :text="__('Cancel')" />
        </x-slot>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <select name="type" class="form-control" required x-on:change="userType = $event.target.value">
                            ...
                            <option value="{{ $model::TYPE_FIGHTER }}" {{ $user->type === $model::TYPE_FIGHTER ? 'selected' : '' }}>@lang('fighter')</option>
                            ...
                            <option value="{{ $model::TYPE_MEMBER }}" {{ $user->type === $model::TYPE_MEMBER ? 'selected' : '' }}>@lang('member')</option>
